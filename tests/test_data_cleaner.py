...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        - Verificar que el DataFrame resultante no tiene valores faltantes en esas columnas (usar self.assertEqual para comparar .isna().sum() con 0 - comparación simple de enteros, 
        unittest es suficiente)
        ...
        tabla_resultado = cleaner.drop_invalid_rows(df_test, ["name", "age"])

        self.assertEqual(tabla_resultado["name"].isna().sum(), 0)
        self.assertEqual(tabla_resultado["age"].isna().sum(), 0)
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        df_test = pd.DataFrame({
            "name": ["   Alice ", "Pedro         ", "   Bob   "],
            "age": [25, 30, None],
            "city": ["SCL", "LPZ", "SCL"]
        })
        cleaner = DataCleaner()

        tabla_resultado = cleaner.trim_strings(df_test, ["name"])

        # Verificar que el DataFrame original no fue modificado
        self.assertEqual(df_test.loc[0, "name"], "   Alice ")
        self.assertEqual(df_test.loc[1, "name"], "Pedro         ")
        self.assertEqual(df_test.loc[2, "name"], "   Bob   ")

        # Verificar que en el DataFrame final los valores no tienen espacios
        self.assertEqual(tabla_resultado.loc[0, "name"], "Alice")
        self.assertEqual(tabla_resultado.loc[1, "name"], "Pedro")
        self.assertEqual(tabla_resultado.loc[2, "name"], "Bob")

        # Verificar que las columnas no usadas permanecen iguales
        pdt.assert_series_equal(tabla_resultado["city"], df_test["city"])
        pdt.assert_series_equal(tabla_resultado["age"], df_test["age"])
    ...
    def test_trim_strings_raises_typeerror_for_non_string_column(self):
        ...
        with self.assertRaises(TypeError):
            cleaner.trim_strings(df_test, ["age"])
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        """
        df_test = pd.DataFrame({
            "name": ["Alice", "Pedro", "Bob", "Ale", "Javier"],
            "age": [25, 30, 35, 50, 120],
            "city": ["SCL", "LPZ", "SCL", "SCL", "SCL"]
        })
        cleaner = DataCleaner()   

        tabla_resultados = cleaner.remove_outliers_iqr(df_test, "age", factor = 1.5)

        self.assertNotIn(120, tabla_resultados["age"].values)
        self.assertIn(35, tabla_resultados["age"].values)
    ...
    def test_remove_outliers_iqr_raises_keyerror_for_missing_column(self):
        ...
        with self.assertRaises(KeyError):
            cleaner.remove_outliers_iqr(df_test, "salary")
